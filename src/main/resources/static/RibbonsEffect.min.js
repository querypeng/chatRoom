"object" == typeof window && (window.Ribbons = function () {
    ...
    return t.prototype = {
        ...
        addRibbon: function () {
            ...
            for (var r = [], l = new b(e, a), c = new b(e, a), p = null, _ = Math.round(u(0, 360)), d = 0; !(i <= 0);) {
                if (i--, s = Math.round((1 * Math.random() - .2) * this._options.horizontalSpeed), h = Math.round((1 * Math.random() - .5) * (.25 * this._height)), (p = new b).copy(c), "right" === t) {
                    if (p.add(s, h), c.x >= o) break
                } else if ("left" === t && (p.subtract(s, h), c.x <= -200)) break;
                r.push({
        ...
        _drawRibbonSection: function (t) {
            if (t) {
                ...
                if (t.delay <= 0) {
                    if (t.phase += .02, t.alpha = 1 * Math.sin(t.phase), t.alpha = t.alpha <= 0 ? 0 : t.alpha, t.alpha = 1 <= t.alpha ? 1 : t.alpha, this._options.animateSections) {
